...
export function SMVSelector({ onSelect, currentSMV }) {
    ...
    <div style={{ position: 'relative', marginBottom: 16 }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <Clock size={14} />
          {loading ? 'Loading articles...' : selected ? `Art# ${selected.article_number || ''} — ${selected.name} (${selected.total_smv} min)` : 'Select article number'}
        </div>
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {filtered.length === 0 ? (
              ...
              Object.entries(grouped).map(([garment, items]) => (
                <div key={garment}>
                  {/* Garment group header */}
                  <div style={{ padding: '6px 14px', fontSize: 10, fontWeight: 600, color: 'var(--text-muted)', background: 'var(--bg)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    {garment}
                  </div>
                  {items.map(t => (
                    <div
