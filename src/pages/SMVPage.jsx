...
const GARMENTS = ['T-Shirt','Polo Shirt','Trousers','Jacket','Jeans','Shorts','Dress','Skirt','Formal Shirt','Hoodie'];
...
export default function SMVPage() {
  ...
  const save = async () => {
    ...
    try {
      await createReport({
        type: 'smv',
        title: `SMV — ${garmentType} — ${new Date().toLocaleDateString()}`,
        inputs: { garmentType, totalOperations: ops.length },
        results: { totalSMV: r.totalSMV, basicTime: r.totalBasicTime, dailyOutput: r.dailyOutputPerOperator, opsFor500: r.operatorsFor500pcs }
      });
  ...
  const saveTemplate = async () => {
    ...
    setSavingTpl(true);
    try {
      await createSMVTemplate({ name: templateName, garment_type: garmentType, operations: ops, total_smv: r.totalSMV });
  ...
  const exportPDF = () => exportReportPDF({
    type: 'smv', title: `SMV Breakdown — ${garmentType}`,
    inputs: { garmentType, operations: ops.length },
    results: { basicTime: r.totalBasicTime + ' min', allowance: r.totalAllowanceTime + ' min', SMV: r.totalSMV + ' min', dailyOutput: r.dailyOutputPerOperator + ' pcs' },
  ...
  return (
    <div>
      <ToastContainer />
      <PageHeader title="SMV / SAM Calculator" subtitle="Build operation breakdown and compute Standard Minute Value" badge={{ text: 'IE Formula' }} />
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div>
              ...
              <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 3 }}>{ops.length} operations · SMV = {r.totalSMV.toFixed(3)} min</p>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <select value={garmentType} onChange={e => setGarmentType(e.target.value)} style={{ width: 'auto', fontSize: 12 }}>
                {GARMENTS.map(g => <option key={g}>{g}</option>)}
              </select>
              <button className="btn btn-primary btn-sm" onClick={() => setOps([...ops, newOp()])}><Plus size={13} /> Add</button>
            </div>
          </div>
          ...
          <div style={{ marginTop: 16, padding: '14px 16px', background: 'var(--bg)', borderRadius: 10, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {[['Basic time', r.totalBasicTime.toFixed(3) + ' min', false], ['Allowance', r.totalAllowanceTime.toFixed(3) + ' min', false], ['SMV / SAM', r.totalSMV.toFixed(3) + ' min', true]].map(([l, v, bold]) => (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="card">
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
              <button className="btn btn-primary" style={{ flex: 1 }} onClick={save} disabled={saving}><Save size={13} />{saving ? 'Saving...' : 'Save report'}</button>
              <button className="btn btn-secondary" onClick={exportPDF}><Download size={13} /></button>
            </div>
          </div>

          <div className="card">
            <h3 style={{ marginBottom: 12 }}><BookOpen size={13} style={{ verticalAlign: -2 }} /> Save template</h3>
            <div className="field"><label>Template name</label><input value={templateName} onChange={e => setTemplateName(e.target.value)} placeholder="Basic T-Shirt ops" /></div>
            <button className="btn btn-secondary btn-full" onClick={saveTemplate} disabled={savingTpl}>
              <Save size={13} />{savingTpl ? 'Saving...' : 'Save to library'}
            </button>
          </div>
        </div>
      </div>
