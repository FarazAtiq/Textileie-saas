...
export function EfficiencyPage() {
  ...
  const { save, doExport, saving, ToastContainer } = useSave(
    'efficiency',
    () => 'Efficiency — ' + new Date().toLocaleDateString(),
    inp, r
  );
    ...
    <div>
      ...
      <CalcGrid>
        <div className="card">
          ...
          <SMVSelector onSelect={t => setInp(p => ({ ...p, smv: t.total_smv }))} />
          <div className="field"><label>Shift duration (minutes)</label><input type="number" value={inp.shiftMinutes} onChange={set('shiftMinutes')} /></div>
          <div className="field"><label>Number of operators</label><input type="number" value={inp.operators} onChange={set('operators')} /></div>
          <div className="field"><label>Units produced</label><input type="number" value={inp.unitsProduced} onChange={set('unitsProduced')} /></div>
        ...
        <ResultCard
          ...
          onSave={save}
          onExport={() => doExport('Efficiency Report')}
          saving={saving}
        />
      </CalcGrid>
      <div className="card" style={{ marginTop: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {[
            ['< 55%', 'Below target', 'var(--red)', 'var(--red-light)'],
            ['55-75%', 'Acceptable', 'var(--amber)', 'var(--amber-light)'],
            ['> 75%', 'World class', 'var(--green)', 'var(--green-light)']
          ].map(([range, lbl, c, bg]) => (
...
export function CapacityPage() {
  ...
  const { save, doExport, saving, ToastContainer } = useSave(
    'capacity',
    () => 'Capacity — ' + new Date().toLocaleDateString(),
    inp, r
  );
    ...
    <div>
      ...
      <CalcGrid>
        <div className="card">
          <h3 style={{ marginBottom: 16 }}>Inputs</h3>
          <SMVSelector onSelect={t => setInp(p => ({ ...p, smv: t.total_smv }))} />
          <div className="field"><label>Machines / operators</label><input type="number" value={inp.machines} onChange={set('machines')} /></div>
          <div className="field"><label>Shifts per day</label><input type="number" value={inp.shiftsPerDay} onChange={set('shiftsPerDay')} /></div>
          <div className="field"><label>Shift duration (minutes)</label><input type="number" value={inp.shiftMinutes} onChange={set('shiftMinutes')} /></div>
          <div className="field"><label>SMV per unit (minutes)</label><input type="number" step="0.1" value={inp.smv} onChange={set('smv')} /></div>
          <div className="field"><label>Target efficiency (%)</label><input type="number" value={inp.efficiencyPct} onChange={set('efficiencyPct')} /></div>
          <div className="field"><label>Working days per month</label><input type="number" value={inp.workingDaysPerMonth} onChange={set('workingDaysPerMonth')} /></div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <ResultCard
            ...
            onSave={save}
            onExport={() => doExport('Capacity Planning Report')}
            saving={saving}
          />
        </div>
      </CalcGrid>
      ...
      {tab === 'yards' && (
        <CalcGrid>
          ...
          <ResultCard
            ...
            mainValue={formatNum(ry.grossYards, 3) + ' yds'}
            mainLabel="Gross yards per unit"
            rows={[
            ...
            onSave={saveY}
            onExport={() => expY('Fabric Consumption (Yards)')}
            saving={savingY}
          />
        </CalcGrid>
      )}
      {tab === 'gsm' && (
        <CalcGrid>
          ...
          <ResultCard
            ...
            mainValue={formatNum(rg.grossWeightKg, 3) + ' kg'}
            mainLabel="Gross weight per unit"
            rows={[
            ...
            onSave={saveG}
            onExport={() => expG('Fabric Consumption (GSM)')}
            saving={savingG}
          />
        </CalcGrid>
      )}
...
export function ThreadPage() {
  ...
  const { save, doExport, saving, ToastContainer } = useSave(
    'thread',
    () => 'Thread — ' + inp.stitchType + ' — ' + new Date().toLocaleDateString(),
    inp, r
  );
    ...
    <div>
      ...
      <CalcGrid>
        ...
        <ResultCard
          ...
          mainValue={formatNum(r.grossMeters, 1) + ' m'}
          mainLabel="Gross thread per garment"
          rows={[
          ...
          onSave={save}
          onExport={() => doExport('Thread Consumption')}
          saving={saving}
        />
      </CalcGrid>
    </div>
...
export function CostingPage() {
  ...
  const { save, doExport, saving, ToastContainer } = useSave(
    'costing',
    () => 'Costing — ' + new Date().toLocaleDateString(),
    inp, r
  );
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
        <div className="card">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {[
              ['fabricCostPerUnit', 'Fabric / unit ($)'],
              ['cmt', 'CMT / unit ($)'],
              ['overhead', 'Overhead / unit ($)'],
              ['freightPerUnit', 'Freight / unit ($)'],
              ['agentCommPct', 'Agent comm (%)'],
              ['bankChargePct', 'Bank charge (%)'],
              ['dutyPct', 'Duty (%)'],
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div className="card">
            ...
            {[
              ['Total production cost', '$' + formatNum(r.totalProductionCost)],
              ['Agent commission', '$' + formatNum(r.agentCommission)],
              ['Bank charge', '$' + formatNum(r.bankCharge)],
              ['Duty', '$' + formatNum(r.duty)],
              ['Profit (' + inp.profit + '%)', '$' + formatNum(r.profitAmount)]
            ].map(([l, v]) => (
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              ...
              <button className="btn btn-secondary" onClick={() => doExport('FOB Costing Sheet')}>
                <Download size={14} />
              </button>
            </div>
          </div>
            ...
            {bars.map(b => (
              <div key={b.label} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                  ...
                  <span style={{ color: b.color, fontFamily: 'JetBrains Mono' }}>
                    {r.fobPrice > 0 ? ((b.v / r.fobPrice) * 100).toFixed(1) : 0}%
                  </span>
                </div>
...
export function YarnPage() {
  ...
  const { save, saving, ToastContainer } = useSave(
    'yarn',
    ...
    <div>
      ...
      <CalcGrid>
        <div className="card">
          ...
          <FormulaNote>Ne = (Length in yards / 840) / (Weight in lbs) | Tex = (g / m) x 1000 | Nm = m / g</FormulaNote>
        </div>
        <ResultCard
          title="Yarn Count Results"
      ...
      <div className="card" style={{ marginTop: 20 }}>
        ...
        <table className="data-table">
          ...
          <tbody>
            {[
              ['Coarse', '6-10', '60-100', '10-17', 'Canvas, denim'],
              ['Medium', '16-30', '20-37', '27-50', 'Shirts, knitwear'],
              ['Fine', '40-60', '10-15', '67-100', 'Voile, poplin'],
              ['Very fine', '80-120', '5-7', '135-200', 'Lawn, batiste']
            ].map(([t, ne, tex, nm, use]) => (
              <tr key={t}>
                <td>{t}</td>
                <td style={{ fontFamily: 'JetBrains Mono', color: 'var(--teal)' }}>{ne}</td>
                <td style={{ fontFamily: 'JetBrains Mono' }}>{tex}</td>
                <td style={{ fontFamily: 'JetBrains Mono' }}>{nm}</td>
                <td style={{ color: 'var(--text-muted)' }}>{use}</td>
              </tr>
            ))}
          </tbody>
        </table>
